...
let customScale = 1;
let targetFps = 60;
let bgmfile = './assets/audio/Theme_341.ogg';
let bgmvolume = 0;
let bgm;
...
const AUDIO_DETAIL = [
	...
	},
	{
		...
		dialog: {
			...
			English: [
				...
				"I intend to survive, no matter what it takes."
			],
			Chinese: [
				"恰恰相反。",
				"不管用什么手段，我都会活下去。"
	...
	},
	{
		time: 15000,
		count: 2,
		startTimes: [3500, 9400],
		dialog: {
			Japanese: [
				"先生がこの世界を見捨てないというのなら。",
				"私だって最後まで、絶対に諦めたりしません。"
			],
			English: [
				"If you won’t abandon this world, Sensei,",
				"then neither will I. Not until the very end."
			],
			Chinese: [
				"如果老师不抛弃这个世界的话。",
				"那我到最后也不会放弃的。"
	...
	},
	{
		...
		dialog: {
			...
			English: [
				"So it’s all right… Truly, it is.",
				"I’ll remain here. Always."
			],
			Chinese: [
				"所以放心吧……绝对，没问题的。",
				"我会一直在这里。"
...
// -1 = [No Entry], 1 = Headpat, 2 = Voiceline, 3 = Eye Track, 4 = Pinch
function pressedMouse(x, y) {
	...
	if (tx > HITBOX.headpat.xMin && tx < HITBOX.headpat.xMax && ty > HITBOX.headpat.yMin && ty < HITBOX.headpat.yMax && mouseOptions.headpatting) {
		...
		// 延迟清空动画轨道，让PinchEnd动画完整播放后再回到idle状态
		setTimeout(() => {
			spineData.state.addEmptyAnimation(1, 0.5, 0);
			spineData.state.addEmptyAnimation(2, 0.5, 0);
			acceptingClick = true;
...
function releasedMouse() {
	switch (mouseSelect) {
		...
		case 3:
			...
			break;
		case 4:
			// Pinch释放 - 不再自动播放结束动画，结束动画已在pressedMouse中自动触发
			// 这里只需要重置mouseSelect状态
			break;
		default:
	}
	mouseSelect = -1;
